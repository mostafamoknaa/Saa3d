...
if(isset($_POST['Login'])){
    ...
    if(empty($user)){
        $log['email']="Please Enter Your email";
    }
    if(empty($pass)){
        $log['password']="Please Enter Your PassWord";
    }
    if(!array_filter($log)){
        $sql="select * from users;";
        ...
        if($res){
            ...
            foreach($users as $std){
                if($std['e_mail'] === $user && $std['password'] === $pass){
                        $_SESSION['user_name']=$std['user_name'];
                        $_SESSION['password']=$std['password'];
                        $_SESSION['id']= $std['user_id'];
                        $_SESSION['add']= $std['address'];
        ...
        if($c==0){
            $error = "Your e-mail or Password is invalid";
        ...
        <main class="mt-5">
            <div class="row">
                <span class="col-xs-1 col-md-2">
                    <a class="logInBtn btn btn-lg btn-none selected">Log In</a>
            ...
            <form class="row needs-validation" action="" method="post">
                ...
                <section class="logIn" style="display:none">
                    <div class="row">
                        <span class="col-xs-1 col-md-4">
                            <input type="email" name = "email" class="form-control logInput" placeholder="E-Mail">
                            <div class="valid-feedback">
                            Looks good!
                            </div>
                            <div class="invalid-feedback">
